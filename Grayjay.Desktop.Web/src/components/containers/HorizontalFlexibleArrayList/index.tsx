...
export interface HorizontalFlexibleArrayListProps {
    items?: any[];
...
const HorizontalFlexibleArrayList: Component<HorizontalFlexibleArrayListProps> = (props) => {
    ...
    const handleScroll = () => {
        ...
        if (!lastChild) {
            console.warn("HorizontalFlexibleArrayList not updated, element does not exist for index", {
                notifyIndex,
                items: props.items,
        ...
        const visibleWidth = outerRect.width + outerRect.left;
        const containerRect = lastChild.getBoundingClientRect();
        if (containerRect.left < visibleWidth) {
            props.onEnd?.();
        }
    };
...
export default HorizontalFlexibleArrayList;
